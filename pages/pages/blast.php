...
<script>
  var cek=0;$("#all").change(function(){this.checked?($("#lists").val([]),$(".thisselect").hide(),$("#lists").attr("disabled",!0),$("#lists").attr("required",!1),cek=1):($("#lists").val([]),$(".thisselect").fadeIn(),$("#lists").attr("disabled",!1),$("#lists").attr("required",!0),cek=0)}),$("#startblast").on("click",a=>{a.preventDefault();var s=[];if($("#lists option:selected").each(function(){s.push($(this).val())}),0==cek&&""==$("#lists").val())return alert("Harap pilih tujuan blast!");if(""==$("#pesan").val())return alert("Pesan wajib diisi!");const e=cek,t=$("#sender").val(),l=$("#pesan").val(),i=($(".delay").val(),$(".urlmedia").val()),r={selected:s,all:e,sender:t,pesan:l,media:i};$(this).attr("disabled",!0),$("#startblast").html('<span class="spinner-grow spinner-grow-sm" role="status" aria-hidden="true"></span>\n                                                Prossess Blasting...'),$("#sender").attr("disabled",!0),$("#pesan").attr("disabled",!0),$("#lists").attr("disabled",!0),$("#all").attr("disabled",!0),$.ajax({type:"POST",url:"../ajax/blast.php",data:r,success:a=>{window.location=""},error:a=>{alert("error contact support ( 123 )")}})});
</script>
<?php
